...
module.exports = {
    create: asyncWrapper(
        async (req, res, next) => {
            ...
            if (newEvent) {
                await cache.setJsonObject('event:'+newEvent.eventID, newEvent)
                await cache.pushJsonToList('events', newEvent)
                await cache.setKeyTTL('events', 600)
                await cache.setKeyTTL('event:'+newEvent.eventID, 600)
    ...
    update: asyncWrapper(
        async (req, res, next) => {
            ...
            if (updatedEvent) {
                ...
                await cache.removeJson('event:'+req.params.eventID, 'events', 'eventHome')
