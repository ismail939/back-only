...
function CreateCoworkingSpace() {
  ...
  const PhoneNumberOneError = () => {
    var numbers = /^[0-9]+$/;
    if (!phonenumberOne.match(numbers)) {
      return true;
    } else if (phonenumberOne.length !== 11) {
      return true;
    } else {
      return false;
    }
  ...
  const CheckRequiredData = () => {
    const data = [Name, Address, Description, phonenumberOne, startDate, endDate]
    data.map((item, index) => {
      if (item.length === 0) {
        return true
      }
  ...
  const HandleError = () => {
    if (CheckRequiredData()) { setCheckError("Please Fill out the required Fields"); window.scrollTo(0, 0); }
    else {
      if (PhoneNumberOneError()) {
        setCheckError("Error: Please write your First phone number correctly"); window.scrollTo(0, 0);
      }
      else if (FirstDateError()) {
